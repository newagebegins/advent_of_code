...
static int findStepsToGoal(Arena* arena, const char* goal, ReplacementList* replacements, AtomNameSet* atomNames)
{
    ...
    constexpr int moleculeSetCapacity = 10000139;
    StringSet moleculeSets[2];
    moleculeSets[0] = makeStringSet(arena, moleculeSetCapacity);
    moleculeSets[1] = makeStringSet(arena, moleculeSetCapacity);

    StringSet* srcSet = &moleculeSets[0];
    StringSet* dstSet = &moleculeSets[1];
    ...
    addStringToSet(srcSet, startMolecule);
    ...
    char decodedMolecule[1024];
    int steps = 0;
    constexpr int debugPrintFrequency = 1;

    while (result < 0)
    {
        time_t now = time(NULL);

        if (steps % debugPrintFrequency == 0)
        {
            printf("\n");
            printf("Steps: %d\n", steps);
            printf("Seconds: %jd\n", now - startTime);
            printf("Arena used: %zu / %zu (%f)\n", arena->used, arena->size, (float)arena->used / arena->size);
            printf("Src set used: %zu / %zu (%f)\n", srcSet->count, srcSet->capacity, (float)srcSet->count / srcSet->capacity);
        ...
        ASSERT(srcSet->count > 0);
        int moleculeCounter = 0;
        for (int i = 0; i < srcSet->capacity && result < 0; ++i)
        {
            char* molecule = srcSet->strings[i];
            if (molecule)
            {
                int moleculeLen = getStringLength(molecule);
                ++moleculeCounter;
                if (steps % debugPrintFrequency == 0 && moleculeCounter <= 10)
                {
                    decodeMolecule(molecule, atomNames, decodedMolecule, ARRAY_COUNT(decodedMolecule));
                    printf("%d) (%d) %s\n", moleculeCounter, moleculeLen, decodedMolecule);
                }
                        ...
                        for (int atomIndex = 0; atomIndex < moleculeLen; ++atomIndex)
                        {
                            if (molecule[atomIndex] == replacement->atom)
                            {
                                ...
                                if (newMoleculeLen <= goalLen)
                                {
                                    ...
                                    bool canReachLeft = newMolecule[0] == goal[0] || canReach(&prefixReachabilityMatrix, newMolecule[0], goal[0]);
                                    bool canReachRight = newMolecule[newMoleculeLen - 1] == goal[goalLen - 1] || canReach(&suffixReachabilityMatrix, newMolecule[newMoleculeLen - 1], goal[goalLen - 1]);
                                    bool isGood = canReachLeft && canReachRight;

                                    if (isGood)
                                    {
                                        if (addStringToSet(dstSet, newMolecule))
                                        {
                                            decodeMolecule(newMolecule, atomNames, decodedMolecule, ARRAY_COUNT(decodedMolecule));
                                        }
                                        else
        ...
        StringSet* tmp = srcSet;
        srcSet = dstSet;
        dstSet = tmp;

        clearStringSet(dstSet);
    }
