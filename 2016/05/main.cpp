...
static void
FindCode(const char *Input, char Code[9])
{
    char ToHash[256];
    uint32_t CodeLength = 0;
    for(uint32_t Suffix = 0;
        CodeLength < 8;
        ++Suffix)
    {
        sprintf_s(ToHash, sizeof(ToHash), "%s%u", Input, Suffix);
        MD5 Hash = getMD5((uint8_t *)ToHash, (uint32_t)strlen(ToHash), sizeof(ToHash));
        if((Hash.bytes[0] == 0) &&
           (Hash.bytes[1] == 0) &&
           (Hash.bytes[2] >> 4) == 0)
        {
            uint32_t N = (Hash.bytes[2] & 0xF);
...
static void
FindCode2(const char *Input, char Code[9])
{
    for(int I = 0; I < 9; ++I)
    {
        Code[I] = 0;
        ...
        ++Suffix)
    {
        ...
        if((Hash.bytes[0] == 0) &&
           ...
           (Hash.bytes[2] >> 4) == 0)
        {
            uint32_t Pos = (Hash.bytes[2] & 0xF);
            if((0 <= Pos) && (Pos < 8) && (Code[Pos] == 0))
            {
                uint32_t N = (Hash.bytes[3] >> 4);
                char C = (char)((N < 10) ? ('0' + N) : ('a' + (N-10)));
                Code[Pos] = C;
                ++Filled;
            }
